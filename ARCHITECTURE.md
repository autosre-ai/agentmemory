...
This document describes the architectural design of Agent Memory Toolkit, explaining key design decisions and component interactions.
...
The extraction module converts unstructured text into structured `Memory` objects across six cognitive domains.

#### Design Decisions

**Six Cognitive Domains**

We chose to categorize memories into six distinct domains based on cognitive science research:
...
- **Rule-based** extraction handles explicit patterns (names, dates, locations)
- **LLM-based** extraction captures implicit information and context
- **Hybrid** mode uses rules first (fast, cheap) then LLM for gaps

**Deduplication Strategy**

We support three deduplication strategies:

1. **Exact** - Hash-based, fastest
2. **Fuzzy** - Jaccard similarity on tokenized text
3. **Semantic** - Embedding cosine similarity (optional)

### 2. Storage Module

The storage module provides persistent, searchable memory storage.

#### Design Decisions

**SQLite as Primary Store**

We chose SQLite over alternatives for several reasons:
...
- **Hybrid** combines both with score normalization

### 3. Security Module

The security module validates memories before storage to prevent poisoning attacks.
...
We defined five security levels to balance security vs. usability:
...
We detect these injection patterns:
...
The compression module manages context window limits intelligently.

#### Design Decisions

**Token-Aware Design**

All compression respects token budgets, not character counts:
...
Messages are ranked by multiple factors:
...
The team module enables multi-agent collaboration with conflict resolution.

#### Design Decisions

**Git-like Model**

We adopted Git's model because:
...
When the same memory is modified by multiple agents:

1. **LATEST_WINS** - Timestamp-based (simple, may lose data)
2. **OURS** - Local version preferred
3. **THEIRS** - Remote version preferred
4. **MERGE** - Attempt automatic merge (metadata combined)
5. **MANUAL** - Raise exception for human resolution
